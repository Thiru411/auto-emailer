...
var KTUsersAddPermission = function () {
    ...
    var selected_client = [];
    for (var option of document.getElementById('clients').options)
        {
            if (option.selected) {
                selected_client.push(option.value);
            }
        }
        var projects = [];
        for (var option of document.getElementById('projects').options)
            {
                if (option.selected) {
                    projects.push(option.value);
                        ...
                        axios.post(AJAX_URL+'users/add_permissions/', {
                            user_id: document.querySelector('[name="users1"]').value, 
                            project_id: selected_client, 
